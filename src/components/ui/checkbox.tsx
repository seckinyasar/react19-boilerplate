...
function CheckboxWithText({ text, disabled = false }: CheckboxWithTextInterface) {
  return (
    <div className="flex items-center space-x-2">
      <Checkbox id="terms" disabled={disabled} />
      <label htmlFor="terms" className={cn('text-base', disabled && 'opacity-50 cursor-not-allowed')}>
        {text}
      </label>
    </div>
  )
