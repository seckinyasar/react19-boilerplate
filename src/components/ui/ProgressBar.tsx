...
const ProgressBar = ({ value, className }: ProgressBarProps) => {
  ...
  return (
    <motion.div className={cn('w-full h-1 rounded-full overflow-hidden', className)}>
      <motion.div className="flex w-full h-full bg-blue-500" style={{ scaleX: value, transformOrigin: 'left' }} />
    </motion.div>
  )
}
